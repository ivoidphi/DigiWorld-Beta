...
public class World {
    ...
    private void buildMap(int tileSize) {
        ...
        int bossNpcTileY = -1;
        int bossNpcTileX = spawnTileX;
        for (Npc npc : npcs) {
            int npcTileY = (int) npc.getY() / tileSize;
            if (bossNpcTileY < 0 || npcTileY < bossNpcTileY) {
                bossNpcTileY = npcTileY;
                bossNpcTileX = (int) npc.getX() / tileSize;
            }
        }
        if (bossNpcTileY >= 0 && bossNpcTileY < spawnTileY) {
            for (int y = bossNpcTileY; y <= spawnTileY; y++) {
                for (int dx = -2; dx <= 2; dx++) {
                    int tx = bossNpcTileX + dx;
                    if (tx >= 0 && tx < getWidth() && y >= 0 && y < getHeight()) {
                        landMask[y][tx] = true;
                        tiles[y][tx] = ((tx + y) % 2 == 0) ? TileType.GRASS1 : TileType.GRASS2;
                    }
                }
            }
        }
